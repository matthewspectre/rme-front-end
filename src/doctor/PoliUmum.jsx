  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (Number(poliVal) !== 1) {
          setAllowed(false)
          setMessage('Akses ditolak: hanya dokter Poli Umum yang dapat membuka halaman ini')
          return
        }
        ...
        const list = (Array.isArray(antrianData) ? antrianData : []).filter(it => (
          (it.idPoli != null && Number(it.idPoli) === 1) ||
          (it.id_poli != null && Number(it.id_poli) === 1) ||
          (it.poli != null && Number(it.poli) === 1) ||
          (it.poli_id != null && Number(it.poli_id) === 1)
        ))
    ...
    <div className="doctor-dashboard">
      ...
      <main className="doctor-main">
        <header className="doctor-header">
          <div>
            <h1 className="doctor-title">Poli Umum — {user.full_name}</h1>
        ...
        <section className="doctor-grid">
          <section className="doctor-card" style={{ width: '100%' }}>
            <div className="card-header">
              <h2>Daftar Antrian Poli Umum</h2>
            </div>
