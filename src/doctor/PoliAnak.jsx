  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        if (Number(poliVal) !== 3) {
          setAllowed(false)
          setMessage('Akses ditolak: hanya dokter Poli Anak yang dapat membuka halaman ini')
          return
        }
        ...
        const list = (Array.isArray(antrianData) ? antrianData : []).filter(it => (
          (it.idPoli != null && Number(it.idPoli) === 3) ||
          (it.id_poli != null && Number(it.id_poli) === 3) ||
          (it.poli != null && Number(it.poli) === 3) ||
          (it.poli_id != null && Number(it.poli_id) === 3)
        ))
    ...
    <div className="doctor-dashboard">
      ...
      <main className="doctor-main">
        <header className="doctor-header">
          <div>
            <h1 className="doctor-title">Poli Anak — {user.full_name}</h1>
        ...
        <section className="doctor-grid">
          <section className="doctor-card" style={{ width: '100%' }}>
            <div className="card-header">
              <h2>Daftar Antrian Poli Anak</h2>
            </div>
